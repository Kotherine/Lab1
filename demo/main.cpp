...
int main() {
  ...
  json data;
  file >> data;
  if (!data.at("items").is_array()) throw std::runtime_error{"The items in the file must be an array!"};
  if (data.at("items").size() != data.at("_meta").at("count").get<size_t>()) throw std::runtime_error{"Value in _meta not equal to the length of the array"};
